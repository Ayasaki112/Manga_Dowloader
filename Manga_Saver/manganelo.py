...
def download_manga(url,remaining):
    ...
    # title and chapter
    page = get_page(url)
    title = page.find('div',{'class':'panel-chapter-info-top'})
    ...
    #image links
    content = page.find('div',{'class':'container-chapter-reader'})
    ...
    f.close()
    if remaining>1:
        next_chapter = page.find('div',{'class':'navi-change-chapter-btn'})
        next_chapter = next_chapter.find('a',{'class':'navi-change-chapter-btn-next a-h'})['href']
